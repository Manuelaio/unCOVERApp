...
for (row in 1:nrow(pre)){
  sel_cc= data.frame(as.character(pre$seqnames[row]), pre$start[row], pre$end[row], stringsAsFactors = F)
...
setwd('./teMpFoldeR/')
write.table(x=for_bed, file =paste0(Sys.Date(),'.bed'), quote=F, sep="\t", eol = "\n", row.names = F,  col.names = F)