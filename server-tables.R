...
coord= eventReactive(input$button1,{
  ...
  shinyjs::show("text1")
  Sys.sleep(0.1)
  my_gene_name=select(org.Hs.eg.db, key= input$Gene_name, columns=c("ENTREZID","GENENAME", "ENSEMBL"), keytype="ALIAS")
  ID=my_gene_name$ENTREZID
  all_gene= data.frame(genes(txdb()))
  pre= do.call(rbind, lapply(ID, function(x) data.frame(subset(all_gene, all_gene$gene_id == x), stringsAsFactors = F)))
...
exon_gp<-eventReactive(input$button1,{
  ...
  eid <- select(org.Hs.eg.db,gname, "ENTREZID", "SYMBOL")[["ENTREZID"]]  
  print(head(eid))
